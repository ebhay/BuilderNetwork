...
import { IdeaRereviewForm } from "@/features/ideas/components/idea-rereview-form";
import { RetryButton } from "@/app/(auth)/ideas/[id]/review/retry-button";
import { PendingReview } from "@/features/ideas/components/pending-review";
    ...
    <main className="flex min-h-screen lg:h-screen w-full flex-col lg:flex-row gap-0 overflow-hidden bg-background">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6 bg-background lg:h-full hide-scrollbar">
        <div className="mx-auto w-full max-w-4xl space-y-6">
          ...
          {idea.visibility === "DRAFT" ? <IdeaRereviewForm idea={idea} /> : null}

          {idea.review_status === "PENDING_REVIEW" ? (
            <>
              <AutoProcessReview ideaId={idea.id} />
              <PendingReview ideaId={idea.id} />
            </>
          ) : null}
